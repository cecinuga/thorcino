...
def create_trainer(in_feature: int, out_feature:int, min_lr: float, max_lr: float, epochs: int, weights_decay:float = 0, bias_decay:float = 0) -> Trainer:
    ...
    loss = MSELoss()
    optimizer = SGD(model.parameters, max_lr, weights_decay, bias_decay)
